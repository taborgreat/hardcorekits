...
/**
 * Soup, the classic Hardcore Games healing item.
 *
...
public final class SoupListener implements Listener {
    ...
    /** bowl + 2 cactus -> mushroom stew, and bowl + 2 cocoa beans -> the same. */
    public static void registerRecipe(Plugin plugin) {
        addStewRecipe(cactusKey(plugin), Material.CACTUS);
        addStewRecipe(cocoaKey(plugin), Material.COCOA_BEANS);
    }

    private static void addStewRecipe(NamespacedKey key, Material ingredient) {
    ...
    public static void unregisterRecipe(Plugin plugin) {
        Bukkit.removeRecipe(cactusKey(plugin));
        Bukkit.removeRecipe(cocoaKey(plugin));
    }

    private static NamespacedKey cactusKey(Plugin plugin) {
        return new NamespacedKey(plugin, "cactus_stew");
    }

    private static NamespacedKey cocoaKey(Plugin plugin) {
        return new NamespacedKey(plugin, "cocoa_stew");
    }
}
