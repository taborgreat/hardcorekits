...
public final class GameConfig {
    ...
    private static List<SwordTier> readSwordTiers(FileConfiguration c) {
        ...
        for (Map<?, ?> entry : c.getMapList("kits.barbarian.tiers")) {
            ...
            if (material == null) {
                Bukkit.getLogger().warning("[HungerGames] Unknown Barbarian sword material '"
     ...
     * needs — {@link gg.hungergames.world.WorldRotator} renames the folder on every restart,
    ...
    public World world() {
        if (!worldName.isBlank()) {
            ...
            Bukkit.getLogger().warning("[HungerGames] Configured world '" + worldName
    ...
    private Location resolveLandCentre(World world) {
        ...
        if (centerPreferSwamp) {
            ...
            if (swamp != null) {
                Bukkit.getLogger().info("[HungerGames] Centre set on a swamp at "
        ...
        if (land != null) {
            Bukkit.getLogger().info("[HungerGames] Centre moved to dry land at "
                    + land.getBlockX() + ", " + land.getBlockZ()
                    + " (configured " + configuredCenterX + ", " + configuredCenterZ + " was water).");
            return land;
        }

        Bukkit.getLogger().warning("[HungerGames] No dry land found near the configured centre; "
